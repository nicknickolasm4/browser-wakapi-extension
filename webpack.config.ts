  ...
  const cfg: webpack.Configuration = {
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        ['process.env.API_URL']: JSON.stringify('https://wakapi.dev/api'),
        ['process.env.CURRENT_USER_API_URL']: JSON.stringify('/compat/wakatime/v1/users/current'),
        ['process.env.HEARTBEAT_API_URL']: JSON.stringify(
          '/compat/wakatime/v1/users/current/heartbeats',
        ),
        ['process.env.LOGOUT_USER_URL']: JSON.stringify('https://wakapi.dev/api'),
